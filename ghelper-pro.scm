...
;;; This version requires eq-properties.scm

(define (make-generic-operator arity #!optional name default-operation)
  ...
  (let ((record (make-operator-record arity)))
    ...
    (eq-put! operator 'type 'generic-operator)
    ;; For backwards compatibility with previous implementation:
    (if (not (default-object? name))
	(begin (set-operator-record-name! record name)
               (eq-put! operator 'name name)))
    operator))

...
(define (make-operator-record arity) (list arity #f '() #f))
...
(define (operator-record-name record) (cadddr record))
(define (set-operator-record-name! record name)
  (set-car! (cdddr record) name)
