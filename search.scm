...
#|
;;; Hypotheticals are a special kind of premise, see tms.scm
;;; This structure is now defined in scmutils/src/solve/solve.scm
(define-structure hypothetical name amb-cell)
|#

;;; However, we need to rename the extra slot:
(define hypothetical-amb-cell hypothetical-extra)
(define set-hypothetical-amb-cell! set-hypothetical-extra!)
...
(define (binary-amb cell)
  (let ((true-premise (make-hypothetical 'true cell))
    ...
    (eq-put! true-premise 'amb-premise me)
    (eq-put! false-premise 'amb-premise me)
...
(define (assimilate-nogood! premise new-nogood)
  (let ((item (delq premise new-nogood))
    ...
    (if (any (lambda (old) (lset<= equal? old item)) set)
        #f
        (let ((subsumed
               (filter (lambda (old) (lset<= equal? item old))
                       set)))
          (set-premise-nogoods! premise
            (lset-adjoin equal?
              (lset-difference equal? set subsumed) item))))))
